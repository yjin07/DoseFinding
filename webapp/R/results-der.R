...
# FIXME: This function is not completed yet
get_der_results_withCovars <- function(df, input, output, addCovar = ~1) {
    type <- ifelse(input$responseType == "Continuous", "gaussian", "binomial")
    fit_der <- fitDERMod(df$Dose, df$Exposure, df$Response, model = input$er_model, type = type, addCovar = addCovar, data = df)
    new_doses <- seq(min(df$Dose), max(df$Dose), by = 2)
    fitted_values2 <- predict(fit_der, newdata = new_doses, type = "response")
