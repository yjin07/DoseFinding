getAddArgs <- function(addArgs, exposures = NULL){
    if(!is.null(exposures)){
        addArgs0 <- list(scal = 1.2*max(exposures), off = 0.01*max(exposures))
    } else {
...
# TODO: Not used in the function
checkAnalyArgs <- function(dose, exposure, resp, data, type,
