...
ui <- dashboardPage(
  dashboardHeader(title = "Exposure Data Simulation", titleWidth = SIDEWIDTH),
  dashboardSidebar(width = SIDEWIDTH,
    sidebarMenu(
      menuItem(" Simulate Data", tabName = "simulate", icon = icon("play")),
...
server <- function(input, output) {
  ...
  observeEvent(input$generate, {
    ...
    # Set quantile factors to ensure quantile labels show up in the correct order when plotting.
    ...
    # Render table and plots
    output$exposure_table <- renderDT({
      datatable(exposure_data[, -c(3:4)])
    })
